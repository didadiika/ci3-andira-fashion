...
class Stok_Model extends CI_Model{
    ...
    var $tabel = "barang, stok_brg";
    var $relasi = "barang.status_data = 'Aktif'";
    var $relasi1 = "barang.id_brg = stok_brg.id_brg";
	var $pilih_kolom = array(
        "*");
	var $order_kolom = array(null, "stok_brg.tanggal_update", "barang.nama_barang", "barang.keterangan", null, null, "stok_brg.sisa");
    ...
    function tampil_data(){
		...
		{
			$this->db->group_start();
			$this->db->like("stok_brg.tanggal_update", $_POST["search"]["value"]);
			$this->db->or_like("barang.nama_barang", $_POST["search"]["value"]);
            $this->db->or_like("barang.keterangan", $_POST["search"]["value"]);
			$this->db->or_like("stok_brg.sisa", $_POST["search"]["value"]);
			$this->db->group_end();
		...
		{
			$this->db->order_by("stok_brg.tanggal_update", "DESC");
		}
    }
    ...
    function tampil_data_barang_masuk(){
        $this->db->select("*");
        $this->db->from("masuk_brg,barang");
        $this->db->where("masuk_brg.id_brg = barang.id_brg");
        $this->db->order_by("masuk_brg.tanggal","desc");
        $this->db->order_by("masuk_brg.id_masuk","desc");
    ...
    function tampil_data_barang_masuk_dengan_id_masuk($id_masuk){
        ...
        $this->db->from("masuk_brg,barang");
        $this->db->where("masuk_brg.id_brg = barang.id_brg");
        $this->db->where("masuk_brg.id_masuk = $id_masuk");
        $hasil = $this->db->get();
        return $hasil;
    }
    ...
    function tampil_masuk_per_barang_per_hari_ini($id_barang){
        ...
        $hasil = $this->db->query("select sum(jumlah) as masuk from masuk_brg where id_brg='$id_barang' and 
